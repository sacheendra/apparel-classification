...
def calculate_features(step_size, num_centroids):	# with nice accuracy at step size = 64 or 80
...
def main():
	...
	potential_num_centroids = [64]

	for i in range(len(potential_step_size)):
		for j in range(len(potential_num_centroids)):
			calculate_features(potential_step_size[i], potential_num_centroids[j])
